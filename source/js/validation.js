...
inputHashtag.addEventListener('input', () => {
  // устанавливаем setCustomValidity пустым, чтобы каждый раз при вызове не сохраняло предыдущую ошибку
  inputHashtag.setCustomValidity('');
  inputHashtag.style.border = 'none';

  // преобразуем к нижнему регистру и удаляем пробелы
  let inputText = inputHashtag.value.toLowerCase().trim();
  if (!inputText) {
    return;
  }

  // преобразуем строку в массив используя в качестве разделителя регулярное выражение "все пробелы - один и более после элемента массива"
  ...
  if (isLongHashtag) {
    inputHashtag.setCustomValidity(
      // eslint-disable-next-line comma-dangle
      'Максимальная длина хэш-тега 20 символов, включая решетку'
    );
  }
